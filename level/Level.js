...
function Level(levelIndex, id) {
    GameObjectList.call(this, id)

    this._levelIndex = levelIndex
    this._waterdrops = new GameObjectList(ID.layer_objects)
    this._enemies = new GameObjectList(ID.layer_objects)
    // this._quitButton = new Button(sprites.button_quit, ID.layer_overlays)

    // this._quitButton.position = new Vector2(Game.size.x - this._quitButton.width - 10, 10)
    ...
    // backgrounds
    var backgrounds = new GameObjectList(ID.layer_background)
    var background_main = new SpriteGameObject(sprites.background_sky, ID.layer_background_1)
    background_main.position = new Vector2(0, Game.size.y - background_main.height)
    backgrounds.add(background_main)

    for (var i = 0; i < 5; i++) {
        var sprid = "mountain_" + (Math.ceil(Math.random()*2))
        var mountain = new SpriteGameObject(sprites[sprid], ID.layer_background_2)
        mountain.position = new Vector2(Math.random() * Game.size.x - mountain.width / 2,
            Game.size.y - mountain.height)
    ...
    // timer
    // var timerBackground = new SpriteGameObject(sprites.timer, ID.layer_overlays)
    // timerBackground.position = new Vector2(10, 10)
    // this.add(timerBackground)
    // var timer = new TimerGameObject(ID.layer_overlays_1, ID.timer)
    // timer.position = new Vector2(25, 33)
    // this.add(timer)
...
Level.prototype.loadTiles = function () {
    ...
    // var hintField = new GameObjectList(ID.layer_overlays)
    // this.add(hintField)
    // var hintFrame = new SpriteGameObject(sprites.frame_hint, ID.layer_overlays_1)
    // hintField.position = new Vector2(hintFrame.screenCenterX, 10)
    // hintField.add(hintFrame)
    // var hintText = new Label("Arial", "14pt", ID.layer_overlays_2)
    ...
    //     var walkLeftButton = new Button(sprites.buttons_player, ID.layer_overlays, ID.button_walkleft)
    //     walkLeftButton.position = new Vector2(10, 500)
    //     this.add(walkLeftButton)
    //     var walkRightButton = new Button(sprites.buttons_player, ID.layer_overlays, ID.button_walkright)
    //     walkRightButton.position = new Vector2(walkRightButton.width + 20, 500)
    //     walkRightButton.sheetIndex = 1
    //     this.add(walkRightButton)
    //     var jumpButton = new Button(sprites.buttons_player, ID.layer_overlays, ID.button_jump)
    //     jumpButton.position = new Vector2(powerupjs.Game.size.x - jumpButton.width - 10, 500)
...
Level.prototype.loadTile = function (tileType, x, y) {
    switch (tileType) {
        ...
        case '@':
            ...
            return this.loadBasicTile(sprites.ground_platform, TileType.platform, false, true);
        case 'X':
...
Level.prototype.loadStartTile = function (x, y) {
    ...
    var startPosition = new Vector2((x + 0.5) * tiles.cellWidth, (y + 1) * tiles.cellHeight)
...
Level.prototype.loadEndTile = function (x, y) {
    var tiles = this.find(ID.tiles)
    var exit = new SpriteGameObject(sprites.goal, ID.layer_objects, ID.exit)
    exit.position = new Vector2(x * tiles.cellWidth, (y + 1) * tiles.cellHeight)
    exit.origin = new Vector2(0, exit.height)
    this.add(exit)
    return new Tile()
}
...
Level.prototype.loadWaterTile = function (x, y) {
    ...
    w.position = new Vector2((x + 0.5) * tiles.cellWidth, (y + 0.5) * tiles.cellHeight - 10)
...
Level.prototype.loadTurtleTile = function (x, y) {
    ...
    enemy.position = new Vector2((x + 0.5) * tiles.cellWidth, (y + 1) * tiles.cellHeight + 25)
    this._enemies.add(enemy)
    return new Tile()
}
...
Level.prototype.loadSparkyTile = function (x, y) {
    ...
    enemy.position = new Vector2((x + 0.5) * tiles.cellWidth, (y + 1) * tiles.cellHeight - 100)
    this._enemies.add(enemy)
    return new Tile()
}
...
Level.prototype.loadFlameTile = function (x, y, enemyType) {
    ...
    var tiles = this.find(ID.tiles)
    enemy.position = new Vector2((x + 0.5) * tiles.cellWidth, (y + 1) * tiles.cellHeight)
    this._enemies.add(enemy)
    return new Tile()
}
